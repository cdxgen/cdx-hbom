...
- **Unsafe privileged enrichment** — `--privileged` / `includePrivilegedEnrichment: true` triggers behavior beyond the documented SMBIOS enrichment flow or unexpectedly broadens host access.
...
- **Vulnerabilities in operating-system tools** — bugs in `system_profiler`, `diskutil`, `ioreg`, `ifconfig`, `lscpu`, `lsblk`, `ip`, `dmidecode`, `lspci`, `lsusb`, `ethtool`, or similar host utilities belong to the OS or tool vendor.
...
- **Expected evidence metadata** — `cdx-hbom` intentionally emits `cdx:hbom:evidence:file*` and `cdx:hbom:evidence:command*` properties to document collection provenance. Reports that these fields exist are not security findings by themselves.
...
| **Least-surprise collection** | Keeping default collection scoped to documented local files and commands        | fixed Linux discovery roots, explicit Darwin command registry, opt-in plist enrichment, opt-in Linux privileged enrichment |
...
| **Privilege management** | Deciding whether Linux privileged enrichment is appropriate                          | Only use `--privileged` when you need SMBIOS enrichment and understand the host permissions involved.                                                        |
