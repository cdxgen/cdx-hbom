...
- **PATH and environment poisoning**: if a local attacker can already control `PATH`, shell startup files, or the `node` process environment, they often already have substantial access. In practice, strict command allowlisting and `PATH` hardening are expected to be enforced by upstream wrappers and integrators around `cdx-hbom`; reports should show a concrete bypass of that documented integration policy to be considered in scope.
...
| **Command restriction**       | Exposing a predictable command plan that upstream tools can enforce             | exported command plans, executed-command tracking, and compatibility hooks that integrators can layer with their own allowlisting and `PATH` controls                                                    |
...
| **Command policy**       | Restricting local command execution where needed                                     | Enforce command allowlisting and a trusted `PATH` in the calling wrapper, CI job, or parent application (for example `cdxgen`) when your environment requires stricter execution policy. |
